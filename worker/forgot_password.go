...
func NewConsumerForgotPasswordWorker(config *config.Config, senderService service.SenderEmailService) *ConsumerForgotPasswordMessage {
...
func (mq ConsumerForgotPasswordMessage) ListenMessageQueue(wg *sync.WaitGroup) {
	...
	log.Printf("[%s] [%s] Comsumer has been connected", "INFO", mq.config.RabbitMQ.ForgotPasswordTopic.RoutingKey)
