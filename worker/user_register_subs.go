...
func NewConsumerUserRegisterWorker(config *config.Config, senderService service.SenderEmailService) *ConsumerUserRegisterMessage {
...
func (mq ConsumerUserRegisterMessage) ListenMessageQueue(wg *sync.WaitGroup) {
	...
	log.Printf("[%s] [%s] Comsumer has been connected", "INFO", mq.config.RabbitMQ.UserRegisterTopic.RoutingKey)
